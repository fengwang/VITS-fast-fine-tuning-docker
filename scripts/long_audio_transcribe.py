...
logging.basicConfig( format='%(asctime)s %(levelname)s: %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', filename='/output/long_audio_transcribe.log', encoding='utf-8', level=logging.DEBUG )

parent_dir = "/output/denoised_audio/"
filelist = list(os.walk(parent_dir))[0][2]
logging.info( f'transcribing {filelist=}' )
...
if __name__ == "__main__":
    ...
    for file in filelist:
        ...
        logging.info(f"transcribing {parent_dir + file}...\n")
        options = dict(beam_size=5, best_of=5)
        transcribe_options = dict(task="transcribe", **options)
        # TODO: check transcribe quality here, and filter out low-quality segments
        ...
        if not os.path.exists("/output/segmented_character_voice/" + character_name):
            os.makedirs("/output/segmented_character_voice/" + character_name)
            logging.info( "creating folder /output/segmented_character_voice/" + character_name)
        ...
        for i, seg in enumerate(result['segments']):
            ...
            savepth = "/output/segmented_character_voice/" + character_name + "/" + wav_seg_name
            speaker_annos.append(savepth + "|" + character_name + "|" + text)
            print(f"Transcribed segment: {speaker_annos[-1]}")
            logging.info(f"Transcribed segment: {speaker_annos[-1]}")
    ...
    with open("/output/long_character_anno.txt", 'w', encoding='utf-8') as f:
        for line in speaker_annos:
            f.write(line)

    logging.info("written to /output/long_character_anno.txt")
    #for line in speaker_annos:
    #    logging.info("written line", line )
